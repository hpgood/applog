...
var applog = (function(window) {
    ...
    function getDevice() {
        ...
        var mb = ['ipad', 'iphone os', 'midp', 'rv:1.2.3.4', 'ucweb', 'android', 'windows ce', 'windows mobile', "ios", "classin", "mqqbrowser", "chrome", "mac", "win", "safari"];
        for (var i in mb) {
    ...
    function getOS() {
        ...
        if (isWin) {
            ...
            var isWin10 = sUserAgent.indexOf("Windows NT 6.2") > -1 || sUserAgent.indexOf("Windows NT 10") > -1 || sUserAgent.indexOf("Windows 10") > -1;
    ...
    return {
        init: function(id, ver, workerPath) {
            try {
                appID = id;
                userID = 0;
                version = ver;
        ...
        _log: function(level, tag, message) {
            try {
                ...
                var _msg = { "type": "message", "data": { "t": timestamp, "l": level, "g": tag, "c": message } };
                if (worker != null) {
                    worker.postMessage(_msg);
                } else {
                    _console_error("@applog worker do not init yet!");
                }
            } catch (err) {
                _console_error(err);
            }
